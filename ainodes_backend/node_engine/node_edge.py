...
from ainodes_backend.node_engine.node_graphics_edge import QDMGraphicsEdge
...
class Edge(Serializable):
    ...
    def __init__(self, scene:'Scene', start_socket:'Socket'=None, end_socket:'Socket'=None, edge_type=EDGE_TYPE_DIRECT):
        ...
        :param scene: Reference to the :py:class:`~node_engine.node_scene.Scene`
        ...
        :Instance Attributes:
            ...
            - **grEdge** - Instance of :class:`~node_engine.node_graphics_edge.QDMGraphicsEdge` subclass handling graphical representation in the ``QGraphicsScene``.
        """
        super().__init__()
        self.scene = scene
    ...
    def start_socket(self):
        ...
        :getter: Returns start :class:`~node_engine.node_socket.Socket`
        :setter: Sets start :class:`~node_engine.node_socket.Socket` safely
        :type: :class:`~node_engine.node_socket.Socket`
        """
    ...
    def end_socket(self):
        ...
        :getter: Returns end :class:`~node_engine.node_socket.Socket` or ``None`` if not set
        :setter: Sets end :class:`~node_engine.node_socket.Socket` safely
        :type: :class:`~node_engine.node_socket.Socket` or ``None``
        """
    ...
    def edge_type(self):
        ...
        :setter: sets new edge type. On background, creates new :class:`~node_engine.node_graphics_edge.QDMGraphicsEdge`
    ...
    def validateEdge(cls, start_socket: 'Socket', end_socket: 'Socket') -> bool:
        ...
        :param start_socket: Starting :class:`~node_engine.node_socket.Socket` of Edge to check
        :type start_socket: :class:`~nodeeditor.node_socket.Socket`
        :param end_socket: Target/End :class:`~node_engine.node_socket.Socket` of Edge to check
    ...
    def getOtherSocket(self, known_socket:'Socket'):
        ...
        :param known_socket: Provide known :class:`~node_engine.node_socket.Socket` to be able to determine the opposite one.
    ...
    def updatePositions(self):
        """
        Updates the internal `Graphics Edge` positions according to the start and end :class:`~node_engine.node_socket.Socket`.
    ...
    def remove_from_sockets(self):
        """
        Helper function which sets start and end :class:`~node_engine.node_socket.Socket` to ``None``
        """
    ...
    def remove(self, silent_for_socket:'Socket'=None, silent=False):
        ...
        Notifies nodes previously connected :class:`~node_engine.node_node.Node` (s) about this event.

        Triggers Nodes':

        - :py:meth:`~node_engine.node_node.Node.onEdgeConnectionChanged`
        - :py:meth:`~node_engine.node_node.Node.onInputChanged`

        :param silent_for_socket: :class:`~node_engine.node_socket.Socket` of a :class:`~node_engine.node_node.Node` which
...
#
# from node_engine.node_edge_validators import *
