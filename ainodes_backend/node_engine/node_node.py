...
from ainodes_backend.node_engine.node_socket import Socket, LEFT_BOTTOM, LEFT_CENTER, LEFT_TOP, RIGHT_BOTTOM, RIGHT_CENTER, RIGHT_TOP
from ainodes_backend.node_engine.utils_no_qt import dumpException
...
class Node(Serializable):
    ...
    def __init__(self, scene: 'Scene', title: str="Undefined Node", inputs: list=[], outputs: list=[]):
        ...
        :param scene: reference to the :class:`~node_engine.node_scene.Scene`
        :type scene: :class:`~nodeeditor.node_scene.Scene`
        :param title: Node Title shown in Scene
        :type title: str
        :param inputs: list of :class:`~node_engine.node_socket.Socket` types from which the `Sockets` will be auto created
        :param outputs: list of :class:`~node_engine.node_socket.Socket` types from which the `Sockets` will be auto created

        :Instance Attributes:

            - **scene** - reference to the :class:`~node_engine.node_scene.Scene`
            - **grNode** - Instance of :class:`~node_engine.node_graphics_node.QDMGraphicsNode` handling graphical representation in the ``QGraphicsScene``. Automatically created in the constructor
            - **content** - Instance of :class:`~node_engine.node_graphics_content.QDMGraphicsContent` which is child of ``QWidget`` representing container for all inner widgets inside of the Node. Automatically created in the constructor
            - **inputs** - list containin Input :class:`~node_engine.node_socket.Socket` instances
            - **outputs** - list containin Output :class:`~node_engine.node_socket.Socket` instances
    ...
    def getNodeContentClass(self):
        """Returns class representing node_engine content"""
    ...
    def initSockets(self, inputs: list, outputs: list, reset: bool=True):
        ...
        for item in inputs:
            ...
            socket = self.__class__.Socket_class(
                node=self, name=socket_name, index=counter, position=self.input_socket_position,
        ...
        for item in outputs:
            ...
            socket = self.__class__.Socket_class(
                node=self, name=socket_name, index=counter, position=self.output_socket_position,
    ...
    def onEdgeConnectionChanged(self, new_edge: 'Edge'):
        ...
        :param new_edge: reference to the changed :class:`~node_engine.node_edge.Edge`
    ...
    def hasConnectedEdge(self, edge: 'Edge'):
        """Returns ``True`` if edge is connected to any :class:`~node_engine.node_socket.Socket` of this `Node`"""
        for socket in (self.inputs + self.outputs):
    ...
    def getSocketPosition(self, index: int, position: int, num_out_of: int=1) -> '(x, y)':
        """
        Get the relative `x, y` position of a :class:`~node_engine.node_socket.Socket`. This is used for placing
    ...
    def getChildrenNodes(self) -> 'List[Node]':
        ...
        :rtype: List[:class:`~node_engine.node_node.Node`]
        """
    ...
    def getInputWithSocket(self, index: int=0) -> [('Node', 'Socket'), (None, None)]:
        ...
        :param index: Order number of the `Input Socket`
        :type index: ``int``
        :return: Tuple containing :class:`~node_engine.node_node.Node` and :class:`~node_engine.node_socket.Socket` which
    ...
    def getInputs(self, index: int=0) -> 'List[Node]':
        ...
        :param index: Order number of the `Input Socket`
        :type index: ``int``
        :return: all :class:`~node_engine.node_node.Node` instances which are connected to the
