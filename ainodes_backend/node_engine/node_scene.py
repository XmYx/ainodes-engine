...
from ainodes_backend.node_engine.node_graphics_scene import QDMGraphicsScene
from ainodes_backend.node_engine.node_node import Node
from ainodes_backend.node_engine.node_edge import Edge
from ainodes_backend.node_engine.node_scene_history import SceneHistory
from ainodes_backend.node_engine.node_scene_clipboard import SceneClipboard
...
class Scene(Serializable):
    ...
    def __init__(self):
        ...
        :Instance Attributes:
            ...
            - **history** - Instance of :class:`~node_engine.node_scene_history.SceneHistory`
            - **clipboard** - Instance of :class:`~node_engine.node_scene_clipboard.SceneClipboard`
    ...
    def addNode(self, node: Node):
        """Add :class:`~node_engine.node_node.Node` to this `Scene`

        :param node: :class:`~node_engine.node_node.Node` to be added to this `Scene`
        :type node: :class:`~nodeeditor.node_node.Node`
        """
        self.nodes.append(node)

    def addEdge(self, edge: Edge):
        """Add :class:`~node_engine.node_edge.Edge` to this `Scene`

        :param edge: :class:`~node_engine.node_edge.Edge` to be added to this `Scene`
        :return: :class:`~node_engine.node_edge.Edge`
        """
        self.edges.append(edge)

    def removeNode(self, node: Node):
        """Remove :class:`~node_engine.node_node.Node` from this `Scene`

        :param node: :class:`~node_engine.node_node.Node` to be removed from this `Scene`
        :type node: :class:`~nodeeditor.node_node.Node`
        """
        if node in self.nodes: self.nodes.remove(node)
        else:
            if DEBUG_REMOVE_WARNINGS: print("!W:", "Scene::removeNode", "wanna remove node_engine", node,
                                            "from self.nodes but it's not in the list!")

    def removeEdge(self, edge: Edge):
        """Remove :class:`~node_engine.node_edge.Edge` from this `Scene`

        :param edge: :class:`~node_engine.node_edge.Edge` to be remove from this `Scene`
        :return: :class:`~node_engine.node_edge.Edge`
        """
    ...
    def setNodeClassSelector(self, class_selecting_function: 'functon') -> 'Node class type':
        ...
        If not set, we will always instantiate :class:`~node_engine.node_node.Node` for each `Node` in the `Scene`
