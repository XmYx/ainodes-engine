from modules.k_diffusion import sampling as k_diffusion_sampling
from modules.extra_samplers import uni_pc
import torch
import collections
from modules.model_loading import model_management
import math
import logging
from modules.sampling import sampler_helpers
import scipy
import numpy
...
def calc_cond_uncond_batch(model, cond, uncond, x_in, timestep, model_options): #TODO: remove
    logging.warning("WARNING: The modules.samplers.calc_cond_uncond_batch function is deprecated please use the calc_cond_batch one instead.")
...
def resolve_areas_and_cond_masks(conditions, h, w, device):
    logging.warning("WARNING: The modules.samplers.resolve_areas_and_cond_masks function is deprecated please use the resolve_areas_and_cond_masks_multidim one instead.")
...
class CFGGuider:
    ...
    def predict_noise(self, x, timestep, model_options={}, seed=None, denoise_mask=None):
        return sampling_function(self.inner_model, x, timestep, self.conds.get("negative", None), self.conds.get("positive", None), self.cfg, model_options=model_options, seed=seed)

    def inner_sample(self, model, noise, latent_image, device, sampler, sigmas, denoise_mask, callback, disable_pbar, seed):
        if latent_image is not None and torch.count_nonzero(latent_image) > 0: #Don't shift the empty latent image.
            latent_image = model.model.process_latent_in(latent_image)

        self.conds = process_conds(model.model, noise, self.conds, device, latent_image, denoise_mask, seed)

        extra_args = {"model_options": self.model_options, "seed":seed}
        if self.inner_model == None or self.inner_model != model.model:
            self.inner_model = model.model
        samples = sampler.sample(self, sigmas, extra_args, callback, noise, latent_image, denoise_mask, disable_pbar)
        return model.model.process_latent_out(samples)#.to(torch.float32))

    def sample(self, model, noise, latent_image, sampler, sigmas, denoise_mask=None, callback=None, disable_pbar=False, seed=None):
        if sigmas.shape[-1] == 0:
            return latent_image
        ...
        _, self.conds, self.loaded_models = sampler_helpers.prepare_sampling(model, noise.shape, self.conds)
        ...
        output = self.inner_sample(model, noise, latent_image, device, sampler, sigmas, denoise_mask, callback, disable_pbar, seed)
