...
def linear_multistep_coeff(order, t, i, j):
    ...
    num_points = 100
    tau_values = np.linspace(t[i], t[i + 1], num_points)
    fn_values = np.array([fn(tau) for tau in tau_values])

    # Compute the integral using the trapezoidal rule
    integral = np.trapz(fn_values, tau_values)
    return integral
...
def sample_euler_cfg_pp(model, x, sigmas, extra_args=None, callback=None, disable=None):
    ...
    extra_args["model_options"] = modules.model_patcher.set_model_options_post_cfg_function(model_options, post_cfg_function, disable_cfg1_optimization=True)
