...
import math
import struct
import modules.model_loading.checkpoint_pickle
...
def load_torch_file(ckpt, safe_load=False, device=None):
    ...
    if ckpt.lower().endswith(".safetensors") or ckpt.lower().endswith(".sft"):
        ...
        if safe_load:
            ...
            pl_sd = torch.load(ckpt, map_location=device, pickle_module=modules.model_loading.checkpoint_pickle)
...
def bislerp(samples, width, height):
    ...
    def generate_bilinear_data(length_old, length_new, device):
        coords_1 = torch.arange(length_old, dtype=torch.float16, device=device).reshape((1,1,1,-1))
        coords_1 = torch.nn.functional.interpolate(coords_1, size=(1, length_new), mode="bilinear")
        ratios = coords_1 - coords_1.floor()
        coords_1 = coords_1.to(torch.int64)
        
        coords_2 = torch.arange(length_old, dtype=torch.float16, device=device).reshape((1,1,1,-1)) + 1
        coords_2[:,:,:,-1] -= 1
...
def lanczos(samples, width, height):
    ...
    images = [torch.from_numpy(np.array(image).astype(np.float16) / 255.0).movedim(-1, 0) for image in images]
