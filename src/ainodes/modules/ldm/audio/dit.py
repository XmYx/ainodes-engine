...
from modules.ldm.modules.attention import optimized_attention
import typing as tp
...
class FourierFeatures(nn.Module):
    ...
    def forward(self, input):
        f = 2 * math.pi * input @ modules.comfy_math.ops.cast_to_input(self.weight.T, input)
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        beta = self.beta
        if beta is not None:
            beta = modules.comfy_math.ops.cast_to_input(beta, x)
        return F.layer_norm(x, x.shape[-1:], weight=modules.comfy_math.ops.cast_to_input(self.gamma, x), bias=beta)
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, t):
        ...
        freqs = torch.einsum('i , j -> i j', t, modules.comfy_math.ops.cast_to_input(self.inv_freq, t))
        ...
        scale = modules.comfy_math.ops.cast_to_input(self.scale, t) ** rearrange(power, 'n -> n 1')
