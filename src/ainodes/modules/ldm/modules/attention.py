...
from ...model_loading import model_management
...
def attention_basic(q, k, v, heads, mask=None, attn_precision=None, skip_reshape=False):
    ...
    if exists(mask):
        if mask.dtype == torch.bool:
            ...
            b, j = mask.shape
            h = 4  # example value for h

            # Repeat the mask tensor along the batch dimension b, h times
            mask = mask.repeat_interleave(h, dim=0)

            # Add a new singleton dimension, resulting in the shape (b*h, 1, j)
            mask = mask_repeated.unsqueeze(1)
