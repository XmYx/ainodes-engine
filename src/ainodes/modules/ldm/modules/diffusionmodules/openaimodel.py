...
from modules.ldm.util import exists
import modules.comfy_math.ops
ops = modules.comfy_math.ops.disable_weight_init
...
class VideoResBlock(ResBlock):
    ...
    def forward(
        ...
        # Reshape x and x_mix from (b * t, c, h, w) to (b, c, t, h, w)
        b, _, h, w = x.size()
        t = num_video_frames
        b = b // t
        x_mix = x.view(b, t, -1, h, w).permute(0, 2, 1, 3, 4)
        x = x.view(b, t, -1, h, w).permute(0, 2, 1, 3, 4)

        # Reshape emb from (b * t, ...) to (b, t, ...)
        emb = emb.view(b, t, -1)

        # Apply time_stack and time_mixer
        x = self.time_stack(x, emb)
        x = self.time_mixer(x_spatial=x_mix, x_temporal=x, image_only_indicator=image_only_indicator)

        # Reshape x back to (b * t, c, h, w)
        x = x.permute(0, 2, 1, 3, 4).reshape(-1, x.size(1), h, w)
        return x
