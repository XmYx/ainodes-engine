...
def model_lora_keys_unet(model, key_map={}):
    ...
    diffusers_keys = modules.utils.unet_to_diffusers(model.model_config.unet_config)
    ...
    if isinstance(model, modules.model_base.SD3): #Diffusers lora SD3
        diffusers_keys = modules.utils.mmdit_to_diffusers(model.model_config.unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    if isinstance(model, modules.model_base.AuraFlow): #Diffusers lora AuraFlow
        diffusers_keys = modules.utils.auraflow_to_diffusers(model.model_config.unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    if isinstance(model, modules.model_base.HunyuanDiT):
        for k in sdk:
            if k.startswith("diffusion_model.") and k.endswith(".weight"):
