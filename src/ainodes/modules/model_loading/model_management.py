...
from modules.cli_args import args
import torch
import sys
import platform
    ...
    def model_load(self, lowvram_model_memory=0, force_patch_weights=False):
        ...
        try:
            if lowvram_model_memory > 0 and load_weights:
                self.model = self.model.patch_model_lowvram(device_to=patch_model_to, lowvram_model_memory=lowvram_model_memory, force_patch_weights=force_patch_weights)
            else:
                self.model = self.model.patch_model(device_to=patch_model_to, patch_weights=load_weights)
        ...
        if is_intel_xpu() and not args.disable_ipex_optimize:
            self.model = ipex.optimize(self.real_model.eval(), graph_mode=True, concat_linear=True)
...
def resolve_lowvram_weight(weight, model, key): #TODO: remove
    print("WARNING: The modules.model_management.resolve_lowvram_weight function will be removed soon, please stop using it.")
    return weight
