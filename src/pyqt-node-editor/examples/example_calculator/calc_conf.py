...
from examples.example_calculator.calc_node_base import CalcNode, CalcGraphicsNode, CalcContent
from generator import FluxGenerator
from modules.ldm.unet.sd import CLIP, VAE
from modules.model_loading.model_base import Flux
...
class AiNode(Node):
    ...
    def __init__(self, scene, title, inputs=[2, 2], outputs=[1]):
    ...
    class ParsedNode(AiNode):
        ...
        def __init__(self, scene, inputs=None, outputs=None):
            ...
            for param_name, param_info in node_data['params'].items():
                ...
                elif param_type in [FluxGenerator, Flux, CLIP, VAE]:
                    inputs.append(9)
                    self.custom_input_socket_names.append(param_name)
            ...
            elif return_type in [FluxGenerator, Flux, CLIP, VAE]:
                outputs.append(9)
        ...
        #     # It should construct the kwargs dictionary and call the original function
        #     param_values = {name: kwargs.get(name) for name in self.named_params}
        #
        #     dynamic_inputs = self.getAllInputs()
        #     print("dyn", dynamic_inputs)
        #     print(param_values)
        #     result = node_data['function'](**param_values)
        #     return [result]
...
class ImageInputWidget(QDMNodeContentWidget):
    fileName = None
    # frame_string_signal = pyqtSignal(str)
    parent_resize_signal = pyqtSignal()
    set_image_signal = pyqtSignal(object)
    def initUI(self):
        self.create_widgets()
...
class ImageInputNode(AiNode):
    ...
    def __init__(self, scene):
        super().__init__(scene, self.op_title, inputs=[1], outputs=[5,1])
        self.images = []
